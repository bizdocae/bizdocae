export default async function handler(req, res) {
  ...
  try {
    ...
    if (!text || String(text).trim().length < 5) return res.status(400).json({ ok:false, error:"Missing or too-short 'text'" });
    ...
    const schema = {
      ...
      properties:{
        ...
        docType:{type:"string",enum:["invoice","proposal","contract","report","financials","bank_statement","marketing","hr","legal","other"]},
        summary:{type:"string"},
        keyEntities:{type:"object",properties:{
        ...
        amounts:{type:"array",items:{type:"object",properties:{
          label:{type:"string"}, value:{type:"number"}, currency:{type:"string"}
        },required:["label","value"]}},
        kpis:{type:"array",items:{type:"object",properties:{
          name:{type:"string"}, value:{type:["number","string"]}, unit:{type:"string"}
        },required:["name","value"]}},
        financialHealth:{type:"object",properties:{
        ...
        actions:{type:"array",items:{type:"object",properties:{
          priority:{type:"integer",minimum:1}, action:{type:"string"}, owner:{type:"string"}, dueDays:{type:"integer"}
    ...
    let parsed; try { parsed = JSON.parse(content); } catch { return res.status(500).json({ ok:false, error:"Model did not return valid JSON", raw: content }); }

    return res.status(200).json({
      ok:true,
      analysis: parsed,
      tokens:{ prompt:data?.usage?.prompt_tokens ?? null, completion:data?.usage?.completion_tokens ?? null }
    });
